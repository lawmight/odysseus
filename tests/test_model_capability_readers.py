...
from src.model_capability_readers import cursor, generic_openai, google, llamacpp, lmstudio, ollama, openai, openrouter
...
def test_cursor_reader_parses_items_shape_identity_only():
    ...
    assert records[0].vendor == VENDOR_CURSOR
    ...
    assert records[0].capability.source == mc.SOURCE_PROVIDER_READER
    assert records[0].capability.confidence == mc.CONFIDENCE_UNKNOWN
    assert records[0].capability_assertions == ()
    assert records[0].deterministic_controls == ()
    assert surfaces(records[0]) == set()

    assert records[1].model_id == "gpt-5.5-high"
    assert records[1].display_name == "GPT 5.5 High"


def test_cursor_reader_parses_legacy_models_shape():
    records = cursor.records_from_payload(
        {
            "models": [
                {"id": "composer-2.5", "displayName": "Composer"},
...
def test_cursor_reader_does_not_infer_capabilities_from_model_id():
    records = cursor.records_from_payload(
        {
            "items": [
                {"id": "composer-2.5-vision", "displayName": "Composer Vision"},
                {"id": "gpt-image-gen", "displayName": "Image Gen"},
            ],
        },
        base_url="cursor://local",
    )
...
def test_registry_uses_cursor_reader_for_cursor_local_endpoint():
    records = readers.records_from_payload(
        {"items": [{"id": "composer-2.5", "displayName": "Composer"}]},
        base_url="cursor://local",
    )
