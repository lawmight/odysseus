...
**Docker on Cloud VMs:** Nested Docker needs `fuse-overlayfs` and `iptables-legacy` ([Cursor setup — Running Docker](https://cursor.com/docs/cloud-agent/setup#running-docker)). `scripts/cloud-agent-docker.sh` (sourced by install/start) installs `docker.io` + **Compose v2** (`docker-compose-v2`) when missing, configures fuse-overlayfs, starts `dockerd`, and prefers `sudo docker` when the user is not in the `docker` group. Without Compose v2, `docker compose up -d` fails with exit **125** (`unknown shorthand flag: 'd'`) and Cursor marks the agent ERROR. Verify with `sudo docker compose version` and `sudo docker run --rm hello-world`. Logs should show `storage-driver=fuse-overlayfs` and `ChromaDB connected`. A notice that sidecars are skipped is **non-fatal** — Chat/Agent still run; only vector memory / SearXNG stay degraded.
