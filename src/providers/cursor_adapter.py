...
def _walk_for_image_path(obj: Any, workspace: str) -> Optional[str]:
    if obj is None:
        return None
    if isinstance(obj, str):
        return _resolve_image_path_candidate(obj, workspace)
    if isinstance(obj, (list, tuple)):
        for item in obj:
            found = _walk_for_image_path(item, workspace)
            if found:
                return found
        return None
    if isinstance(obj, dict):
        for key in _IMAGE_PATH_KEYS:
            if key in obj:
                found = _walk_for_image_path(obj[key], workspace)
                if found:
                    return found
        for value in obj.values():
            found = _walk_for_image_path(value, workspace)
            if found:
                return found
    return None
...
def extract_generate_image_path(result: Any, workspace: str) -> Optional[str]:
    """Best-effort local path extraction from an untyped Cursor tool result."""
    return _walk_for_image_path(result, workspace)
...
def publish_cursor_generated_image(
    *,
    ...
    """Copy Cursor-generated bytes into Odysseus gallery storage and return SSE fields."""
    ...
    try:
        ...
        try:
            db.add(
                GalleryImage(
                    id=image_id,
                    filename=filename,
                    prompt=(prompt or "Cursor generateImage")[:500],
        ...
        logger.warning("Failed to save Cursor generated image to gallery", exc_info=True)
        image_id = ""

    out: Dict[str, str] = {
        "image_url": image_url,
        "image_prompt": prompt or "Cursor generateImage",
        "image_model": model or "cursor",
    }
    if image_id:
