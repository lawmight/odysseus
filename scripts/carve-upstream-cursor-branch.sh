...
Options:
  ...
  --dry-run             Fetch, validate refs, and print the plan (no branch changes)
...
upstream Copilot code paths in app.py, src/llm_core.py, src/endpoint_resolver.py
(keep both providers working; do not delete either side). Manifest files may lag
APIs that newer upstream code imports (e.g. resolve_endpoint_runtime in
src/endpoint_resolver.py) — reconcile those too; the test run below catches them.
...
if [[ $DRY_RUN -eq 1 ]]; then
  ...
  if [[ ${#dry_missing[@]} -gt 0 ]]; then
    echo "dry-run: warning — paths missing from ${SOURCE}:"
    printf '  %s\n' "${dry_missing[@]}"
  fi
  exit 0
fi
...
if [[ $DO_PUSH -eq 0 ]]; then
  echo "carve-upstream-cursor-branch: OK (local commit; review diff, reconcile Copilot/manifest drift, then --push)"
  exit 0
fi

if [[ $CARVED -eq 0 ]]; then
  echo "carve-upstream-cursor-branch: refusing --push — no carve commit was created; pushing would reset ${FORK_BRANCH} to bare ${UPSTREAM_REF}" >&2
  exit 1
fi
